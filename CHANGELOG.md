...
## v1.3.3 — 日次サマリー孤児トリガー削除メニューを追加 (2026-06-18)

### Added
- カスタムメニュー「SSD管理」に「日次サマリーの孤児トリガー削除」を追加。`deleteDailySummaryTrigger()` が `sendDailySummary` の残存時間トリガーを検出して削除する
- v1.3.2 で無効化した日次サマリーの孤児トリガーを、GASエディタのトリガー画面を開かずメニューから1クリックで掃除できるようにした（`DAILY_SUMMARY_ENABLED=false` の安全装置はそのまま）
