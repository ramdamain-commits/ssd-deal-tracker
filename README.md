...
PS5対応SSD 5機種（1TB）の価格を自動追跡し、目標価格を下回ったらメール通知するシステム。
...
| フィルタ軸 | 選択肢 | 説明 |
|-----------|--------|------|
| メーカー | 全て / WD / Crucial / KIOXIA | 製品名からブランドを自動判定して絞り込む |
| ステータス | 全て / 買い時 / 高め / 未取得 | 目標価格との比較結果で絞り込む。「未取得」は取得エラーの製品を含む |

- 絞り込み条件は URL クエリパラメータ（例: `?brand=crucial&status=買い時`）に反映され、リンク共有やブックマークに対応する
...
| `migration202604e()` | v1.2.0 ラインナップ見直し（9製品削除、5製品に絞り込み、target_price 更新） |
